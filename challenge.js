...
function snakeHitbody() {
	for (var index = 0; index < snakeSegments.length; index += 1) {
		if (snakeSegments[index].x==snakeHead.x &&  snakeSegments[index].y==snakeHead.y ) {
		noLoop();
		}
}
	// print("what's snakeSegments[index]:  ", snakeSegments[index].x);
	// 	print("snake head:  ", snakeHead.x);
